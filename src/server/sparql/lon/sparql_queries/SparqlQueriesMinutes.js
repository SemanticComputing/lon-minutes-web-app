...
UNION
{
    ?id linguistics:referenceToPerson/:refers_to ?person__id .
    ?person__id skos:prefLabel ?person__prefLabel 
    BIND(CONCAT("/people/page/", REPLACE(STR(?person__id), "^.*\\\\/(.+)", "$1")) AS ?person__dataProviderUrl)
}
UNION
{
    ?id linguistics:referenceToOrganization/:refers_to ?organization__id .
    ?organization__id skos:prefLabel ?organization__prefLabel 
    BIND(CONCAT("/people/page/", REPLACE(STR(?organization__id), "^.*\\\\/(.+)", "$1")) AS ?organization__dataProviderUrl)
}
UNION
{
    ?id linguistics:referenceToLocation/:refers_to ?location__id .
...
UNION
{
    ?id linguistics:referenceToDate/:refers_to ?date__id .
    ?date__id skos:prefLabel ?date__prefLabel 
    BIND(CONCAT("/times/page/", REPLACE(STR(?date__id), "^.*\\\\/(.+)", "$1")) AS ?date__dataProviderUrl)
}
UNION
{
    ?id linguistics:referenceToMiscellaneous/:refers_to ?miscellaneous__id .
    ?miscellaneous__id skos:prefLabel ?miscellaneous__prefLabel 
    BIND(CONCAT("/places/page/", REPLACE(STR(?miscellaneous__id), "^.*\\\\/(.+)", "$1")) AS ?miscellaneous__dataProviderUrl)
}
`
