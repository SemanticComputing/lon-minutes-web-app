...
export const personPropertiesInstancePage = `
...

  {
    ...
    BIND (CONCAT("/people/page/", REPLACE(STR(?id), "^.*\\\\/(.+)", "$1")) AS ?label__dataProviderUrl)
  }
  UNION
  ...
  { 
    ...
    UNION
    {
      ...
      BIND (CONCAT("/people/page/", REPLACE(STR(?acquaintance__id), "^.*\\\\/(.+)", "$1")) AS ?acquaintance__dataProviderUrl)
